...
mod tests {
    ...
    ///
    /// The shared builder leaves `proposer_lookahead` zeroed, since that field
    /// is fulu-specific and most of this crate's per-fork test states never
    /// touch it; this is the caller that does, so it runs the real
    /// computation on top before handing the state back.
