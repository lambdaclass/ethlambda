...
mod tests {
    ...
    use crate::primitives::{ExecutionAddress, ExecutionBlockHash, Root, Uint256};

    /// An all-zero execution payload header, standing in for the genesis
    /// payload: nothing under test here inspects it, so only its presence
    /// matters for building a state.
    ...
    /// which is private to that file's own test module; every per-fork test
    /// module in this crate keeps its own copy of its fork's builder rather
    /// than sharing one across files (see, for example,
    /// `crate::stf::epoch::capella::tests::capella_state_with_validators`'s
    /// own doc comment), and this is another instance of that.
