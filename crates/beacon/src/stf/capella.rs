...
mod tests {
    ...
    /// which builds the same shape of state one fork earlier; this crate still
    /// has no shared per-fork test-state builder, so each fork's own test
    /// module grows another near-duplicate of the last one.
