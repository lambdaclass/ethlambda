...
/// Test-only; every test that flips a global must call this (and hold the
/// serialization lock).
#[cfg(test)]
pub(crate) fn reset_for_test() {
    FAKE_ENABLED.store(false, Ordering::Relaxed);
    AGG_RATE.store(0, Ordering::Relaxed);
    VERIFY_RATE.store(0, Ordering::Relaxed);
    MERGE_RATE.store(0, Ordering::Relaxed);
}

/// Serializes every test in this crate that mutates the process-global
/// atomics above (shared with `lib.rs`'s fake-interception tests).
#[cfg(test)]
...
mod tests {
    ...
    #[test]
    fn delay_is_zero_when_rate_unset() {
        let _guard = TEST_LOCK.lock().unwrap();
        reset_for_test();

        init(false, None, None, None);
        assert_eq!(aggregate_delay(100), Duration::ZERO);
        assert_eq!(verify_delay(100), Duration::ZERO);
        assert_eq!(merge_delay(100), Duration::ZERO);

        reset_for_test();
    }

    #[test]
    fn delay_is_zero_when_rate_non_positive() {
        let _guard = TEST_LOCK.lock().unwrap();
        reset_for_test();

        init(false, Some(0.0), Some(-5.0), Some(f64::NAN));
        assert_eq!(aggregate_delay(100), Duration::ZERO);
        assert_eq!(verify_delay(100), Duration::ZERO);
        assert_eq!(merge_delay(100), Duration::ZERO);
    ...
    #[test]
    fn delay_is_zero_when_n_is_zero() {
        let _guard = TEST_LOCK.lock().unwrap();
        reset_for_test();

        init(false, Some(100.0), Some(100.0), Some(100.0));
        assert_eq!(aggregate_delay(0), Duration::ZERO);
        assert_eq!(verify_delay(0), Duration::ZERO);
        assert_eq!(merge_delay(0), Duration::ZERO);

        reset_for_test();
    }

    #[test]
    fn delay_is_proportional_to_n_over_rate() {
        let _guard = TEST_LOCK.lock().unwrap();
        reset_for_test();

        // rate = 100 units/sec, n = 100 units -> ~1 second.
        init(false, Some(100.0), None, None);
        let delay = aggregate_delay(100);
        assert!(
            delay > Duration::from_millis(990) && delay < Duration::from_millis(1010),
            "expected ~1s, got {delay:?}"
        );
    ...
    #[test]
    fn delay_scales_with_operation_count() {
        let _guard = TEST_LOCK.lock().unwrap();
        reset_for_test();

        init(false, None, Some(50.0), Some(200.0));

        // verify: 50 units/sec, 25 units -> ~0.5s.
        let verify = verify_delay(25);
        assert!(
            verify > Duration::from_millis(490) && verify < Duration::from_millis(510),
            "expected ~0.5s, got {verify:?}"
        );

        // merge: 200 units/sec, 100 units -> ~0.5s.
        let merge = merge_delay(100);
        assert!(
            merge > Duration::from_millis(490) && merge < Duration::from_millis(510),
            "expected ~0.5s, got {merge:?}"
        );
    ...
    #[test]
    fn fill_fake_proof_produces_requested_length() {
        for len in [0usize, 1, 7, 64, 1000] {
            let proof = fill_fake_proof(len, &[b"seed"]);
            assert_eq!(proof.len(), len);
        }
    }

    #[test]
    fn fill_fake_proof_at_fake_proof_size_fits_bytelist_cap() {
        let proof = fill_fake_proof(FAKE_PROOF_SIZE, &[b"seed"]);
        assert_eq!(proof.len(), FAKE_PROOF_SIZE);
        assert!(proof.len() < 512 * 1024);
    }

    #[test]
    fn fill_fake_proof_is_deterministic_for_identical_seeds() {
        let a = fill_fake_proof(256, &[b"hello", b"world"]);
        let b = fill_fake_proof(256, &[b"hello", b"world"]);
        assert_eq!(a.iter().as_slice(), b.iter().as_slice());
    }

    #[test]
    fn fill_fake_proof_differs_when_a_seed_part_changes() {
        let a = fill_fake_proof(256, &[b"hello", b"world"]);
        let b = fill_fake_proof(256, &[b"hello", b"there"]);
        assert_ne!(a.iter().as_slice(), b.iter().as_slice());
    }

    #[test]
    fn fake_xmss_defaults_to_false() {
        let _guard = TEST_LOCK.lock().unwrap();
        reset_for_test();
