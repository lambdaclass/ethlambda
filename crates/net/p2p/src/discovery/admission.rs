...
///
/// `accepts` runs inside the peer table's message loop, so it must stay cheap;
/// [`admit`] only parses entries out of a record it was handed, and the fork id
/// and committee count are captured here at construction.
#[derive(Clone)]
