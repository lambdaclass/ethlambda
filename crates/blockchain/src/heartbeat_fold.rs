...
mod tests {
    use super::*;
    use ethlambda_types::{checkpoint::Checkpoint, state::PUBLIC_KEY_SIZE};
    ...
    /// A placeholder signature for tests that only need
    /// `ValidatorSignature::from_bytes` to succeed. `resolve_fold_job` never checks
    /// validity, only that the signature clones and carries a resolvable id —
    /// mirrors `aggregation::tests::dummy_sig`. An all-zero blob decodes as a
    /// valid (unverifiable) signature.
