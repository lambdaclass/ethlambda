...
/// `block`, `finalized_checkpoint`, `chain_reorg`, `attestation`,
/// `block_gossip`); `justified_checkpoint`, `safe_target`, and `aggregate` are
/// ethlambda extensions with no direct beacon topic.
...
impl EventBus {
    ...
    /// Publish an event whose payload is only worth building when someone is
    /// listening. The closure runs (and any clone/allocation it does) only past
    /// the no-subscriber guard, so high-rate emitters (`attestation`,
    /// `aggregate`) pay nothing on an idle node. Same best-effort delivery as
    /// [`EventBus::emit`].
    pub fn emit_with(&self, event: impl FnOnce() -> ChainEvent) {
        if self.tx.receiver_count() == 0 {
            return;
        }
        let _ = self.tx.send(event());
...
mod tests {
    ...
    /// `emit_with` never runs its payload closure when there are no subscribers,
    /// so high-rate emitters pay nothing on an idle node.
    #[test]
    fn emit_with_skips_closure_without_subscribers() {
        let bus = EventBus::default();
        let mut built = false;
        bus.emit_with(|| {
            built = true;
        ...
        assert!(!built, "payload closure ran despite no subscribers");
    }

    /// With a live subscriber, `emit_with` builds and delivers the event.
    #[tokio::test]
    async fn emit_with_delivers_when_subscribed() {
        let bus = EventBus::default();
        let mut sub = bus.subscribe();
        bus.emit_with(|| head_event(9));
        match sub.recv().await.unwrap() {
            ChainEvent::Head { slot, .. } => assert_eq!(slot, 9),
            other => panic!("unexpected event: {other:?}"),
        }
    }
}
