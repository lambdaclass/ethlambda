...
/// plumbing. Consequences of diffing at this level, both intentional:
///
/// - Multiple head moves within one store call coalesce into a single `head`
///   event; subscribers only care about the latest.
/// - The proposer's pre-build catch-up (`get_proposal_head`) runs outside any
///   snapshot window, so a head move never surfaces before the block it
///   belongs to exists; the block's own import then emits the final head.
struct ChainEventSnapshot {
    head: H256,
...
mod tests {
    ...
    /// Insert a header-only block at `root` so head-header reads resolve.
    ...
    fn chain_event_diff_skips_head_with_missing_header() {
        ...
        insert_test_block(&mut store, finalized_root, 1, genesis);
        ...
        match rx.try_recv().unwrap() {
            ChainEvent::FinalizedCheckpoint { slot, root } => {
                assert_eq!((slot, root), (1, finalized_root));
            }
    ...
    /// A head within the recency window emits normally, mirroring
    /// `chain_event_diff_emits_moves_in_order` but stated explicitly against
    /// the gate for clarity.
    #[test]
    ...
    /// Boundary case: `new_head_slot + HEAD_EVENT_RECENCY_SLOTS >=
    /// wall_clock_slot` is the emit condition, so equality still emits, while
    /// one slot further pushes it over into "gated".
    #[test]
    fn chain_event_diff_head_recency_boundary() {
        let head_slot = 1;

        // Exactly at the boundary: emits (`>=` includes equality).
        {
            let mut store = test_store();
            let genesis = store.head().expect("store head exists");
            let bus = EventBus::new(8);
            ...
            let new_root = H256([9u8; 32]);
            insert_test_block(&mut store, new_root, head_slot, genesis);
            store
                .update_checkpoints(ForkCheckpoints::head_only(new_root))
                .expect("update_checkpoints should succeed");

            let wall_clock_slot = head_slot + HEAD_EVENT_RECENCY_SLOTS;
            snapshot.diff_and_emit(&store, &bus, wall_clock_slot);

            match rx.try_recv().unwrap() {
                ChainEvent::Head { slot, .. } => assert_eq!(slot, head_slot),
                other => panic!("expected head event at the boundary, got: {other:?}"),
            }
            assert!(matches!(rx.try_recv(), Err(TryRecvError::Empty)));
        }

        // One slot past the boundary: gated.
        {
            let mut store = test_store();
            let genesis = store.head().expect("store head exists");
            let bus = EventBus::new(8);
            ...
            let wall_clock_slot = head_slot + HEAD_EVENT_RECENCY_SLOTS + 1;
            snapshot.diff_and_emit(&store, &bus, wall_clock_slot);
