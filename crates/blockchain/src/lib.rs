...
impl BlockChainServer {
    ...
    async fn propose_block(&mut self, slot: u64, validator_id: u64) {
        ...
        // aggregate can be built independently of the block root.
        let head_state = self.store.head_state();
        ...
        // the proposer signature stands alone, mirroring `(prop-sig, empty-proof)`.
        let attestation_proof = if single_message_aggregates.is_empty() {
            ...
            // Merge yields raw lean-multisig multi-message aggregate bytes.
            // Per-component participants are rederived at verify time from
            ...
            let merged_bytes = match ethlambda_crypto::merge_type_1s_into_type_2(merge_inputs) {
                Ok(bytes) => bytes,
                Err(err) => {
                    error!(%slot, %validator_id, %err, "Failed to merge single-message aggregates into multi-message aggregate");
                    metrics::inc_block_building_failures();
                    return;
                }
            };
