...
pub fn process_block(state: &mut State, block: &Block) -> Result<(), Error> {
    ...
    process_execution_payload(state, block)?;
...
fn process_execution_payload(state: &mut State, block: &Block) -> Result<(), Error> {
...
mod execution_payload_tests {
    use super::*;
    use ethlambda_types::{
        block::BlockBody, execution_payload::ExecutionPayloadV3, state::Validator,
    };
